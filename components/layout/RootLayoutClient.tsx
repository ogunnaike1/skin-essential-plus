...
export function RootLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Skip splash entirely for returning visitors in the same session
  ...
  return (
    <>
      {/* Content is ALWAYS in the DOM — hero image preload starts at t=0 */}
      {children}
        ...
        {splashVisible && (
          ...
          >
            ...
            <motion.div
              ...
              className="absolute top-1/2 right-1/3 w-72 h-72 rounded-full bg-gradient-to-br from-deep/50 to-transparent blur-3xl"
            />
            ...
            {[...Array(12)].map((_, i) => {
              ...
              const startY = (i * 13.7) % 100;
              return (
                <motion.div
                  key={i}
                  initial={{ x: `${startX}%`, y: `${startY}%` }}
                  animate={{ y: [`${startY}%`, `${startY - 10}%`, `${startY}%`], opacity: [0, 1, 0] }}
