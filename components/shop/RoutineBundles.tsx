...
import { getPublicBundles } from "@/lib/supabase/bundles-api";
 ...
 * Now fetches from Supabase and supports add to cart.
 */
export function RoutineBundles(): React.ReactElement {
  ...
  const handleAddBundle = (bundle: Bundle, bundleProducts: Product[]) => {
    ...
    showSuccess("cart-added", {
      message: `${bundle.name} bundle added to your cart`
    });
  };
  ...
  const savings = active.original_price - active.bundle_price;
  const savingsPercent = Math.round(
    (savings / active.original_price) * 100
  );

  // Map product IDs to actual products
  const bundleProducts = active.product_ids
                ...
                <div className="relative aspect-[4/5] lg:aspect-auto lg:h-[640px]">
                  {active.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-mauve-tint">
                      <Gift className="h-24 w-24 text-mauve" strokeWidth={1.5} />
                    </div>
                  )}
                  ...
                  <div className="flex-1 flex flex-col p-6 sm:p-8 lg:p-10">
                        ...
                        {bundleProducts.map((product, idx) => (
                          ...
                          >
                            <div className="relative h-14 w-14 shrink-0 rounded-xl overflow-hidden bg-deep-tint">
                              {product.image_url ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center">
                                  <Sparkles className="h-6 w-6 text-deep/20" />
                                </div>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              >
                                {product.key_ingredient} · {product.volume}
                              </p>
                    ...
                    <div className="mt-auto pt-6 border-t-2 border-deep/10">
                      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
                        <div>
                          <div className="flex items-baseline gap-3 mb-1">
                            <span className="text-sm text-deep/60 line-through tabular-nums">
                              {formatShopPrice(active.original_price)}
                            </span>
                            <span
                              className={cn(
                          ...
                          <div className="flex items-baseline gap-2">
                            ...
                            >
                              {formatShopPrice(active.bundle_price)}
                            </span>
                        ...
                        <button
                          type="button"
                          onClick={() => handleAddBundle(active, bundleProducts)}
                          className={cn(
